...
def generate_ome_tiff(infile, outfile):
    ...
    with tempfile.TemporaryDirectory(dir=TMPDIR) as tmpdirname:
        ...
        subprocess.run([BIOFORMATS2RAW_CMD, '--resolutions=1', '--tile_height=1024', '--tile_width=1024'] +
                       [infile, n5_file],
                       env=BF_ENV, check=True, capture_output=True, universal_newlines=True)
        logger.info('converting to ome-tiff')
        subprocess.run([RAW2OMETIFF_CMD, '--rgb', '--compression=LZW'] + [n5_file, ome_file],
...
def z_metadata(ome_xml, series, z):
        ...
        if e.tag == "{http://www.openmicroscopy.org/Schemas/OME/2016-06}Pixels":
            ...
            for p in e:
                if p.tag == "{http://www.openmicroscopy.org/Schemas/OME/2016-06}Plane":
                    ...
                    new_pixel.append(
                        ET.Element(p.tag,
                                   {k: (series['SizeC'] if k == 'PlaneCount' else v) for k, v in e.attrib.items()})
                    )
                else:
    ...
    for e in root:
        if e.tag == "{http://www.openmicroscopy.org/Schemas/OME/2016-06}Image":
            ...
            z_root.append(e)
    return ET.ElementTree(z_root)


def split_tiff_by_z(infile, series, zplane, ome_metadata, compression='lzw'):
    """
    ...
    :param zplane:
    :param ome_metadata:
    :param compression: Compression method to use: lzw or jpeg
    :return:
    """
    outfile = Z_OME_FILE.format(file=infile, s=series['Number'], z=zplane)

    if compression.lower() not in ['lzw', 'jpeg']:
        logger.info('Compression method must be lzw or jpeg')
    logger.info(f'Generating Z plane {zplane} from {infile}  using {compression} compression')
    vips_convert = [
                       VIPS_CMD, 'tiffsave',
                       f"{infile}.ome.tif[page={series['Number']}, n={len(series['Channels'])}]",
                       outfile,
                       '--compression=lzw',
                       '--properties',  # Make sure there is a ImageDescription tag
                       '--bigtiff',  # Doesn't hurt to make bigtiff by default
    ...
    if result.stdout:
        logger.info(result.stdout)
    set_omexml(outfile, z_metadata(ome_metadata, series, zplane))
...
def seadragon_tiffs(image_path, z_planes=None, overwrite=True, delete_ome=True, compression='lzw'):
    """
    ...
    :param compression: Compression method to use on z-stack OME-TIFF files
    :return:
    """
...
def main(imagefile, overwrite=False, compression='lzw'):
    try:
        start_time = time.time()
        seadragon_tiffs(imagefile, overwrite=overwrite, compression=compression)
